...
export class AgensApiService {
  ...
  // request = { 
  //   "vid" : "match path=(a:production {'title': 'Haunted House'})-[]-(b:company {'name': 'Ludo Studio'}) return path limit 10" 
  // };
  dbNeighbors( vid:string ){
    const url = `${this.apiUrl}/neighbors`;
    var headers = this.auth.createAuthorizationHeader(this.auth.getToken());
    console.log( `try dbNeighbors ==> ${url} (vid='${vid}')`);
  ...
  dbSaveProject(request:any){
    ...
    let headers = this.auth.createAuthorizationHeader(this.auth.getToken());
    console.log( `try dbSaveProject ==> ${url}`);
